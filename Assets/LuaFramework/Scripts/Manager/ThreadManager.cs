...
namespace LuaFramework {
    ...
    public class ThreadManager : Manager {
		...
		private Dictionary<string, Thread> m_threads = new Dictionary<string, Thread> ();

		public void AddThreadEvent (String name, LuaFunction func)
		{
		...
		public Boolean StartThreadEvent(String name)
		{
			if(m_threads.ContainsKey(name))
			{
				m_threads [name].Start ();
				return true;
			}
			return false;
		}

		public Boolean StopThreadEvent(String name)
		{
			if(m_threads.ContainsKey(name))
			{
				m_threads [name].Abort ();
				return true;
			}
			return false;
		}
